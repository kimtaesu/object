...
public class NightlyDiscountPhone: Phone {
    ...
    public init(nightlyAmount: Money, regularAmount: Money, seconds: TimeInterval, taxRate: Double) {
        ...
        super.init(amount: regularAmount, seconds: seconds, taxRate: taxRate)
    }
    ...
    public override func calculateFee() -> Money {
        ...
        for call in calls {
            ...
            if hour >= NightlyDiscountPhone.LATE_NIGHT_HOUR {
                ...
                result = result.minus(nightlyFee.plus(nightlyFee.times(Double(call.duration.seconds / seconds.seconds))))
            }
        }
